...
macOS (work, Genesys) and Arch/i3 (personal desktop), managed with GNU stow and
...
`~/.devopsclirc` and the Kiro CLI shell files are generated by their own tools and
are sourced, never inlined, so their updates do not churn this repo.

## Git identity

Work is the default, since everything in `~/repos` is Genesys. Personal repos are
matched on their **remote URL**, not their path, because this repo sits in
`~/repos` alongside work:

```
~/repos/dotfiles     -> nedsi <nedsi@pm.me>
~/repos/watchtower   -> Julien Sudan <julien.sudan@genesys.com>
```
